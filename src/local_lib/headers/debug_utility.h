...
#define DEBUG_SIGNAL(mex, signum) fprintf(stderr, "%s%s [DEBUG]:= | file_in: %s | pid: %d | line %d |\n -> %s\n%s", COLOR_RESET_ANSI_CODE, COLOR_WHITE_ANSI_CODE, __FILE__, getpid(), __LINE__, mex,signum, COLOR_RESET_ANSI_CODE)
#define DEBUG_ERROR_MESSAGE(mex) fprintf(stderr, "%s%s [DEBUG-SIGNAL]:= | file_in: %s | pid: %d | line %d |\n -> %s, {%d}\n%s",COLOR_RESET_ANSI_CODE, COLOR_YELLOW_ANSI_CODE, __FILE__, getpid(), __LINE__,mex,COLOR_RESET_ANSI_CODE)
#endif
