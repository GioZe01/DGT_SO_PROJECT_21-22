...
#include <unistd.h>
typedef union proc_type *ProcInfo;
struct processses_info_list{
    pid_t pid;
    short int proc_state;
    short int proc_type;
    float budget;/*TODO: verificare se basta*/
    ProcInfo proc;
    struct processses_info_list* next;
};
/**
 ...
 */
struct processses_info_list* insert(struct processses_info_list* self, pid_t pid, short int type, ProcInfo proc);
/**
 ...
 * @param self ref of the list to print
 */
