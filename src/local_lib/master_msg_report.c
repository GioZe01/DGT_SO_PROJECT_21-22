        ...
        case IMPOSSIBLE_TO_CONNECT_TO_SHM:
            switch (self->state) {
                ...
                case PROC_STATE_NODE_SERV_TRANS:
                    printf("[MASTER_MSG_REPORT]  := type: %s%s%d%s | sender: %d | state: %s%s%d%s \n", COLOR_RESET_ANSI_CODE,
