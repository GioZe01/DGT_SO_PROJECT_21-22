...
void print_cashflow(struct user_transaction *self) {
    printf("[%d]-> budget: %f, ENTRIES: %f, OUTCOMES: %f, expected_out: %f  \n", getpid(), self->budget,
           self->cash_flow.entries, self->cash_flow.outcomes, self->expected_out);
}
...
#ifdef DEBUG_USER
            DEBUG_NOTIFY_ACTIVITY_RUNNING("UPDATE CASH FLOW...");
#endif
            if (self->update_cash_flow(self, t) < 0) {
                return -1;
            }
#ifdef DEBUG_USER
            DEBUG_NOTIFY_ACTIVITY_DONE("UPDATE CASH FLOW DONE");
#endif
